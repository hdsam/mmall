...
public class ProductServiceImpl implements IProductService {
	...
	private ProductDetailVo assmbleProductDetailVo(Product product) {
		...
		// imageHost
		// 设计上传图片存放主机
		productDetailVo.setImageHost(PropertiesUtil.getProperty("ftp.server.http.prefix", "http://server1/"));
		...
		// createTime updateTime java.util.Date格式转化为String
...
//		pageResult.setList(productListVoList);
	...
	private ProductListVo assembleProductListVo(Product product) {
		...
		productListVo.setImageHost(PropertiesUtil.getProperty("ftp.server.http.prefix", "http://server1/"));
	...
	public ServerResponse<PageInfo> getProductByKeywordCategory(String keyword,Integer categoryId, int pageNum, int pageSize,String orderBy){
		...
		if(StringUtils.isNotBlank(orderBy)) {
			if(Const.ProductListOrderBy.PRICE_ASC_DESC.contains(orderBy)) {
				String orderByStr = orderBy.replace("_", " ");
